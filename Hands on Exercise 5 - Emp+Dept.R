...
##################### Creating a DataFrame of the Managers and their Employees

Manager_subset <- subset(emp_dept_merged, select = c("MGR", "ENAME"))
colnames(emp_dept_merged)

emp_dept_merged[,c(3, 5)]

Manager_temp <- emp_dept_merged[,c(3, 5)]

Ordered_MGR <- Manager_temp[order('mgr$MGR', decreasing = T)]


Ordered_MGR <- Manager_temp[,c("ENAME","MGR")]
Ordered_MGR_Final <- Ordered_MGR[order(Ordered_MGR$MGR,decreasing = TRUE),]
View(Ordered_MGR_Final)

##################### End Creating a DataFrame of the Managers and their Employees
